...
#### 1. 黑盒测试
黑盒测试将程序视为一个黑盒子，测试人员不关心程序内部结构和工作原理，只关注输入和输出。黑盒测试的目标是通过运行与可能的程序错误相关的测试用例来发现缺陷。黑盒测试的主要变种包括等价类划分和边界值分析。

#### 2. 等价类划分 (Equivalence Partitioning)
等价类划分是一种黑盒测试技术，它将程序的输入域划分为若干个数据类，每个数据类代表了一组有效的或无效的输入条件。通过这种方法，可以减少测试用例的数量，同时确保覆盖所有可能的输入情况。

- **步骤**：
  1. **识别等价类**：对每个输入条件进行分析，并将其划分为两个或多个组，定义有效和无效的等价类。
  2. **生成测试用例**：为每个等价类设计测试用例。
...
- **启发式规则**（用于识别等价类）：
  1. **数值范围**：例如，如果计数器的范围是1到999，则有效类为`1 <= 计数器 <= 999`，无效类为`计数器 < 1` 和 `计数器 > 999`。
  2. **值的数量**：例如，一辆车最多可以有6个车主，则有效类为`1 到 6 个车主`，无效类为`没有车主` 和 `超过6个车主`。
  3. **逻辑条件**：例如，标识符的第一个字符应该是字母，则有效类为`标识符以字母开头`，无效类为`标识符不以字母开头`。
  4. **输入值集合**：例如，车辆类型可以是`公交车`、`卡车`、`轿车`或`摩托车`，则每个类型都是一个有效类，其他任何类型的车辆为无效类。
  5. **特殊情况**：如果认为程序不会平等对待等价类中的所有项，则应将等价类进一步细分为更小的等价类。

#### 3. 边界值分析 (Boundary Value Analysis)
边界值分析是一种专注于边界条件的测试技术，不仅选择有效和无效等价类中的任意值作为测试用例，还特别关注输入和输出域的边界值。

- **规则**：
  1. **数值范围**：对于范围`[min, max]`，设计测试用例分别针对`min`、`max`、`min-1` 和 `max+1`。
  2. **值的数量**：对于最小值`min`和最大值`max`，设计测试用例分别针对`min`、`max`、`min-1` 和 `max+1`。
  3. **有序集**：如果输入是一个有序集，特别注意第一个和最后一个元素。
  4. **输出数据**：同样适用于输出数据，确保边界值也被测试。

#### 4. 示例
文件中给出的一个示例是银行应用程序，用户可以执行某些银行交易。为了访问交易信息，需要提供以下输入：
- **银行代码**：空白或三位数字，第一位数字必须大于1。
- **分行代码**：四位数字，第一位数字必须大于0。
- **账户号码**：五位数字。
- **密码**：五位字母数字组合。
- **订单**：空白或字符串“Checkbook”或“Transactions”。

通过对这些输入条件进行等价类划分和边界值分析，可以生成一系列测试用例，确保覆盖所有可能的输入情况，包括有效和无效的输入。

### 总结
黑盒测试技术，特别是等价类划分和边界值分析，是确保软件质量的重要手段。通过合理划分等价类和关注边界条件，测试人员可以有效地减少测试用例的数量，同时提高测试的覆盖率和效率。
